...
class ZoneGeographiqueController extends ChangeNotifier {
  ZoneGeographiqueController(this._repository) {
  ...
  Future<bool> creer({required String libelle, String? parentId}) => _executer(
        () => _repository.creer(libelle: libelle, parentId: parentId),
      );

  Future<bool> desactiver(String id) => _executer(() => _repository.desactiver(id));

  Future<bool> supprimer(String id) => _executer(() => _repository.supprimer(id));
